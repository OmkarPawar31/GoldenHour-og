...
export default function OperatorLogin() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      saveAuth(data.token, data.user?.role || "organizer");
  ...
  return (
    <div ref={pageRef}>
      <style>{`
        ...
        .gh-login-hero { background: linear-gradient(150deg, #1e0f2d 0%, #2b1141 60%, #170824 100%); }
        .gh-hero-c { background: radial-gradient(circle, #8B5CF6 0%, transparent 70%); }
        .gh-hero-badge { color: #8B5CF6; background: rgba(139,92,246,0.12); border-color: rgba(139,92,246,0.3); }
        .gh-hero-icon { filter: drop-shadow(0 0 32px rgba(139,92,246,0.5)); }
        .gh-hero-headline em { color: #8B5CF6; }
        .gh-hero-sub { color: #a78bfa; }
        .gh-hero-stat-val { color: #8B5CF6; }
        .gh-input:focus { border-color: #8B5CF6; box-shadow: 0 0 0 3px rgba(139,92,246,0.08); }
        .gh-submit { background: linear-gradient(135deg, #8B5CF6 0%, #A855F7 100%); box-shadow: 0 4px 20px rgba(139,92,246,0.28); }
        .gh-submit:hover { box-shadow: 0 8px 32px rgba(139,92,246,0.4); }
      `}</style>
      <div className="gh-login-root">
        <div className="gh-login-hero">
          ...
          <div className="gh-hero-badge">Event Coordination · Operator Access</div>
          <div className="gh-hero-icon">📋</div>
          <h2 className="gh-hero-headline">Event<br /><em>Operator</em> Portal</h2>
          <p className="gh-hero-sub">Coordinate with emergency services and monitor event safety networks in real time.</p>
          <div className="gh-hero-stats">
            <div><div className="gh-hero-stat-val">SYNC</div><div className="gh-hero-stat-label">Services Sync</div></div>
            <div><div className="gh-hero-stat-val">LIVE</div><div className="gh-hero-stat-label">Status Monitor</div></div>
            <div><div className="gh-hero-stat-val">EVENT</div><div className="gh-hero-stat-label">Control Level</div></div>
          </div>
        </div>
        <div className="gh-login-form-panel">
          <div className="gh-form-wrap">
            ...
            <p className="gh-form-sub">Access operator dashboard</p>
            <form onSubmit={handleAuth}>
              ...
              <button type="submit" className="gh-submit" disabled={loading}>{loading && <span className="gh-spinner-login" />}{loading ? "Authenticating…" : "Sign In to Dashboard →"}</button>
            </form>
            <div className="gh-divider-line"><span /><em>OPERATOR ACCESS</em><span /></div>
