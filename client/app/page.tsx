    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Left dark panel */
        .hero-left { position: relative; background: linear-gradient(150deg, #0a0f1e 0%, #0F172A 60%, #1a0500 100%); display: flex; flex-direction: column; justify-content: center; padding: 7rem 5rem 5rem; overflow: hidden; }
        ...
        .hero-badge { display: inline-flex; align-items: center; gap: 0.6rem; background: rgba(232,87,26,0.12); border: 1px solid rgba(232,87,26,0.3); border-radius: 100px; padding: 0.35rem 1rem; font-family: 'JetBrains Mono', monospace; font-size: 0.68rem; color: var(--orange); letter-spacing: 0.08em; margin-bottom: 1.8rem; width: fit-content; animation: fadeUp 0.7s 0.2s ease both; }
        .badge-dot { width: 6px; height: 6px; background: var(--orange); border-radius: 50%; animation: blink 1.2s infinite; }
        @keyframes blink { 0%,100%{opacity:1} 50%{opacity:0.2} }

        .hero-heading { font-family: 'Bebas Neue', cursive; font-size: clamp(4rem, 7vw, 6.5rem); line-height: 0.92; letter-spacing: 0.02em; color: #fff; margin-bottom: 1.4rem; animation: fadeUp 0.7s 0.4s ease both; }
        .hero-heading em { color: var(--orange); font-style: normal; display: block; }
        .hero-sub { color: #94A3B8; font-size: 1rem; line-height: 1.78; max-width: 420px; margin-bottom: 2.2rem; animation: fadeUp 0.7s 0.6s ease both; }

        .hero-actions { display: flex; gap: 1rem; flex-wrap: wrap; animation: fadeUp 0.7s 0.8s ease both; }
        .btn-primary-hero { padding: 0.95rem 2.2rem; border-radius: 12px; border: none; cursor: pointer; background: linear-gradient(135deg, var(--orange), var(--orange2)); color: #fff; font-family: 'DM Sans', sans-serif; font-size: 0.95rem; font-weight: 700; text-decoration: none; transition: all 0.25s; box-shadow: 0 4px 24px rgba(232,87,26,0.35); display: inline-flex; align-items: center; gap: 0.5rem; }
        .btn-primary-hero:hover { transform: translateY(-3px); box-shadow: 0 10px 36px rgba(232,87,26,0.5); }
        .btn-outline-hero { padding: 0.95rem 2.2rem; border-radius: 12px; cursor: pointer; border: 1.5px solid rgba(255,255,255,0.15); background: rgba(255,255,255,0.05); color: #fff; font-family: 'DM Sans', sans-serif; font-size: 0.95rem; font-weight: 600; text-decoration: none; transition: all 0.25s; display: inline-flex; align-items: center; gap: 0.5rem; }
        .btn-outline-hero:hover { border-color: rgba(232,87,26,0.5); background: rgba(232,87,26,0.08); transform: translateY(-2px); }

        .hero-stats { display: flex; gap: 2.5rem; margin-top: 2.8rem; animation: fadeUp 0.7s 1s ease both; padding-top: 2rem; border-top: 1px solid rgba(255,255,255,0.07); }
        .stat-num { font-family: 'Bebas Neue', cursive; font-size: 2.1rem; color: var(--orange); display: block; line-height: 1; }
        .stat-label { font-family: 'JetBrains Mono', monospace; font-size: 0.62rem; color: #475569; text-transform: uppercase; letter-spacing: 0.1em; margin-top: 0.25rem; }

        @keyframes fadeUp { from{opacity:0;transform:translateY(28px)} to{opacity:1;transform:translateY(0)} }

        /* Right cream panel */
        .hero-right { position: relative; background: var(--cream); display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 3rem; overflow: hidden; }
        ...
        .rp-card { display: flex; align-items: center; gap: 0.7rem; padding: 0.8rem 1rem; border-radius: 12px; border: 1.5px solid var(--border); background: #fff; text-decoration: none; color: var(--text); font-size: 0.82rem; font-weight: 600; transition: all 0.2s; box-shadow: 0 2px 8px rgba(0,0,0,0.04); }
        .rp-card:hover { border-color: var(--accent, var(--orange)); transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,0,0,0.08); }
        ...
        .section-label { font-family: 'JetBrains Mono', monospace; font-size: 0.7rem; color: var(--orange); text-transform: uppercase; letter-spacing: 0.15em; margin-bottom: 0.8rem; font-weight: 600; }
        .section-title { font-family: 'Bebas Neue', cursive; font-size: clamp(2.4rem,5vw,3.8rem); line-height: 1; letter-spacing: 0.02em; color: var(--text); margin-bottom: 1rem; }
        .section-title span { color: var(--orange); }
        .reveal { opacity: 0; transform: translateY(32px); transition: opacity 0.7s ease, transform 0.7s ease; }
        .reveal.visible { opacity: 1; transform: translateY(0); }

        /* Features */
        .features-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 1px; background: var(--border); border: 1px solid var(--border); border-radius: 20px; overflow: hidden; margin-top: 3.5rem; box-shadow: 0 4px 32px rgba(0,0,0,0.06); }
        @media (max-width: 900px) { .features-grid { grid-template-columns: 1fr; } }
        .feature-card { background: #fff; padding: 2.5rem; transition: all 0.3s; position: relative; overflow: hidden; }
        .feature-card::before { content:''; position:absolute; top:0; left:0; right:0; height:3px; background:linear-gradient(90deg,var(--orange),var(--amber)); transform:scaleX(0); transform-origin:left; transition:transform 0.4s; }
        ...
        .feature-badge { display:inline-block; margin-top:0.9rem; font-family:'JetBrains Mono',monospace; font-size:0.63rem; color:var(--orange); background:rgba(232,87,26,0.08); border-radius:4px; padding:0.2rem 0.55rem; border:1px solid rgba(232,87,26,0.15); }

        /* Roles */
        .roles-grid { display:grid; grid-template-columns:repeat(5,1fr); gap:1.2rem; margin-top:2.5rem; }
        @media (max-width: 1100px) { .roles-grid { grid-template-columns:repeat(3,1fr); } }
        @media (max-width: 768px) { .roles-grid { grid-template-columns:1fr 1fr; } }
        .role-card { border:1.5px solid var(--border); border-radius:14px; padding:2rem 1.5rem; background:#fff; cursor:pointer; transition:all 0.35s; text-decoration:none; color:var(--text); display:block; position:relative; overflow:hidden; box-shadow:0 2px 12px rgba(0,0,0,0.04); }
        .role-card::after { content:''; position:absolute; bottom:0; left:0; right:0; height:3px; transform:scaleX(0); transform-origin:left; transition:transform 0.3s; background:var(--accent-color,var(--orange)); }
        ...
        .step { display:grid; grid-template-columns:60px 1fr; gap:1.5rem; padding:1.8rem 0; border-bottom:1px solid var(--border); align-items:start; transition:all 0.2s; }
        .step:last-child { border-bottom:none; } .step:hover { transform:translateX(4px); }
        ...
        .step-tag { display:inline-block; margin-top:0.55rem; font-family:'JetBrains Mono',monospace; font-size:0.63rem; color:var(--muted); background:var(--slate); border-radius:4px; padding:0.2rem 0.55rem; border:1px solid var(--border); }

        /* CTA dark band */
        .cta-band { position: relative; background: linear-gradient(135deg, #0a0f1e 0%, #0F172A 60%, #1a0500 100%); overflow: hidden; margin: 0 3rem 8rem; border-radius: 28px; box-shadow: 0 20px 80px rgba(0,0,0,0.18); }
        ...
        /* Footer */
        .site-footer { border-top:1px solid var(--border); padding:2.5rem 3rem; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:1rem; background:#fff; }
      ...
      <section className="hero">
        <div className="hero-left">
          ...
          <div className="hero-actions">
            <Link href="/login/ambulance" className="btn-primary-hero">⚡ Get Started</Link>
        ...
        <div className="hero-right">
          ...
          <div className="radar-box">
            ...
            <div className="blip blip-amb" /><div className="blip blip-c1" /><div className="blip blip-c2" /><div className="blip blip-c3" /><div className="blip blip-priv" />
            <div className="blip-label">🚑 AMB-001</div>
          </div>
          <div className="radar-portals">
            <Link href="/login/ambulance" className="rp-card" style={{ ["--accent" as string]: "#E8571A" }}><span className="rp-icon">🚑</span><div><div>Ambulance</div><div className="rp-sub">P-100 ACCESS</div></div></Link>
            <Link href="/login/hospital"  className="rp-card" style={{ ["--accent" as string]: "#3B82F6" }}><span className="rp-icon">🏥</span><div><div>Hospital</div><div className="rp-sub">FLEET CONTROL</div></div></Link>
      ...
      <section id="roles" className="section">
        ...
        <h2 className="section-title reveal">Five Roles, <span>One Mission</span></h2>
        <div className="roles-grid reveal">
          {[
            { icon: "🚑", name: "Ambulance Driver",  desc: "Activate emergency mode, get fastest route, terminate on arrival.",            priority: "PRIORITY 100 — MAXIMUM", color: "#E8571A", href: "/login/ambulance" },
      ...
      <section id="how-it-works" className="section">
        <div className="steps-grid">
          ...
          <div style={{ display: "flex", flexDirection: "column" }} className="reveal">
            {[
        ...
        <div className="cta-inner">
          <div style={{ flex: 1 }}>
            ...
            <div className="cta-actions">
              <Link href="/login/ambulance" className="btn-primary-hero" style={{ padding: "1rem 2.5rem", fontSize: "1rem" }}>⚡ Get Started Now</Link>
